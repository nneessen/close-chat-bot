...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      if (existingEvent) {
        console.log(
          "⚠️ Duplicate webhook detected, using existing event:",
          existingEvent.id,
        );
      ...
      if (
        ...
        console.log(
          "🔄 Duplicate constraint error, continuing with mock webhook event...",
        );
        webhookEvent = {
    ...
    if (
      payload.event.object_type === "activity.sms" &&
    ...
    return NextResponse.json({
      success: true,
      eventId: webhookEvent?.id || "processed",
    });
  } catch (error) {
